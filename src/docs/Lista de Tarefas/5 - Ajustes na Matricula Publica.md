# Tarefa — Ajustes na Matrícula Pública (/matricula)

**Repositório:** `rastreio-frontend`
**Depende de:** nada. Independente de todas as outras tarefas desta leva.
**Execução:** Codex só executa — o arquivo completo abaixo já foi escrito e validado com `tsc --noEmit` e `eslint` pelo orquestrador. Não há nada para planejar.

## Por que o arquivo completo, e não localizar/substituir

As mudanças pedidas nesta tela (separar em telas, trocar terminologia, adicionar o toggle do nível misto, reformatar telefone) se entrelaçam bastante — o mesmo estado (`curso`, `anoSelecionado`, `nivelMistoSelecionado`) é lido e escrito em vários pontos do arquivo, e a divisão em telas reestrutura o `return` inteiro. Editar por partes aqui teria mais risco de deixar uma tag JSX mal fechada do que ajuda. Por isso este documento traz o **arquivo `MatriculaPublicPage.tsx` completo, já pronto para substituir o arquivo atual por inteiro**.

## O que muda, resumido

1. **Duas telas em vez de uma.** Ao abrir `/matricula`, em vez de ver a consulta de solicitação e o formulário de matrícula ao mesmo tempo, agora aparecem 2 botões — "Fazer matrícula" e "Consultar matrícula feita" — e cada um leva à tela correspondente. Um botão "Voltar" no topo leva de volta a este menu (antes, "Voltar" saía da página; agora só faz isso quando já se está no menu).
2. **Terminologia dos níveis escolares corrigida**, no card que mostra a instituição selecionada (Passo 1 do formulário): "Ensino Primário e Iº Ciclo" para o fundamental, "Ensino Primário, Iº Ciclo, e Médio" para o misto, "Ensino Médio" e "Ensino Superior" nos outros casos — em vez do nível cru que vinha do backend (ex.: "Escola fundamental", "Escola misto").
3. **Toggle para instituições de nível misto com cursos** (Passo 2): antes, os anos do fundamental e os do médio apareciam misturados numa lista só de "Ano acadêmico". Agora aparece primeiro um toggle — "Ensino Primário ou Iº Ciclo" (habilita só 1ª–9ª Classe) ou "Ensino Médio" (habilita os anos do médio do curso escolhido, que passa a ser obrigatório escolher primeiro nesse caso).
4. **Documentos extra da academia continuam a aparecer corretamente** de acordo com o ano/nível escolhido — o mecanismo que já existia (filtra pelo `anoSelecionado`) não foi tocado, só confirmado que continua funcionando com o toggle novo.
5. **Telefone com formatação visual** ("000 000 000") nos campos de busca de solicitação e de telefone para pagamento GPO, na tela de consulta — mesmo padrão (`maskTelefoneAngola`) que já era usado no Passo 4 do formulário.

**Decisão registada:** o botão "Ir para a Alteração de Senha"/link semelhante não existe nesta tela — não se aplica aqui (essa observação é da tarefa de e-mails do backend, não desta).

---

## Arquivo completo — substituir integralmente

**Caminho:** `src/app/(full-width-pages)/(auth)/matricula/MatriculaPublicPage.tsx`

**Arquivo completo — `/home/claude/work/rastreio-frontend/src/app/(full-width-pages)/(auth)/matricula/MatriculaPublicPage.tsx`:**
...
Depois de substituir o arquivo por completo pelo conteúdo acima e confirmar `tsc`/`eslint` limpos, marcar esta tarefa como concluída.
